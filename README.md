...
<img width="1273" alt="image" src="https://github.com/user-attachments/assets/6246dd04-8c4e-4017-8633-a28b6ada2a41" />

Метод получения информации по всем ПВЗ работает довольно медленно (в среднем 1-1,5 секунды), я думаю, что это обуславливается транзакцией, в которой вызываются очень много методов БД. 
...
Такой метод я сделал, и дополнительную ручку для него GET /pvz/optimized. Скорость работы в среднем стала 0.5-1,5 секунды, то есть прирост есть. Однако этот метод я сделал только ради эксперемента, поэтому ни unit, ни интеграционными тестами я его не покрывал.
